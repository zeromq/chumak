...
-module(erlangzmq_acceptance_pair).
...
start_worker(Identity,  Func) ->
    ...
    spawn_link(
      fun () ->
              {ok, Socket} = erlangzmq:socket(pair, Identity),
              {ok, _PeerPid} = erlangzmq:connect(Socket, tcp, "localhost", ?PORT),
...
negotiate_without_multipart(Socket) ->
    ...
    timer:sleep(200),
    ok = erlangzmq:send(Socket, <<"Hey brother">>),
...
negotiate_without_timeout(Socket) ->
    ...
    ok = erlangzmq:send(Socket, <<"Hey mother">>),
...
negotiate_with_delay(Socket) ->
    ...
    ok = erlangzmq:send(Socket, <<"Hey father">>),
